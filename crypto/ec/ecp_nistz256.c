...
typedef struct p256_point_afine_st P256_POINT_AFFINE;
                                   ...
                                   const BIGNUM *scalars[], BN_CTX *ctx)
{
    ...
    if (no_precomp_for_generator) {
        ...
        memcpy((void *) new_scalars, (const void*) scalars, num * sizeof(BIGNUM *));
        new_scalars[num] = scalar;
		memcpy((void *)new_points, (const void*)points, num * sizeof(EC_POINT *));
...
err:
    ...
    BN_CTX_free(new_ctx);
    if (new_points)
		OPENSSL_free((void *) new_points);
    if (new_scalars)
		OPENSSL_free((void *) new_scalars);
    return ret;
}
